...
interface DynamicFormDeleteDialogProps {
  open: boolean;
  formName: string;
...
export function DynamicFormDeleteDialog({
  open,
  formName,
  onConfirm,
  onCancel,
  isLoading = false,
}: DynamicFormDeleteDialogProps) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-slate-900 shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-red-600 to-rose-600 dark:from-red-900 dark:to-rose-900 p-6 flex items-center gap-3">
          ...
          <h2 className="flex-1 text-lg font-bold text-white">Delete Form</h2>
          <button
            onClick={onCancel}
            disabled={isLoading}
        ...
        <div className="p-6 space-y-4">
          <p className="text-slate-600 dark:text-slate-300">
            ...
            <span className="font-semibold text-slate-900 dark:text-white">{formName}</span>? This
            action cannot be undone and will remove all associated fields.
          </p>
