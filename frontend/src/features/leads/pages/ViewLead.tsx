    ...
    return (
      <div className="space-y-8">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Lead Not Found</h1>
        </div>
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 dark:border-slate-600 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900/50 dark:to-slate-800/50 p-12 text-center">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">This lead does not exist</h3>
          <p className="text-slate-600 dark:text-slate-400 mb-6">The lead you are looking for could not be found</p>
          <button
            onClick={() => navigate("/leads")}
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{lead.leadName}</h1>
            <p className="text-slate-600 dark:text-slate-400">{lead.companyName}</p>
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-gradient-to-r from-blue-600 via-cyan-500 to-blue-600 dark:from-blue-900 dark:via-cyan-900 dark:to-blue-900 p-8 shadow-lg overflow-hidden relative">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-white mb-2">Contact: {lead.contactPerson}</h2>
              <p className="text-blue-100">Follow-up: {new Date(lead.followUpDate).toLocaleDateString()}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-blue-100 mb-1">Expected Deal Value</p>
              <p className="text-3xl font-bold text-white">${lead.expectedDealValue.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </div>
